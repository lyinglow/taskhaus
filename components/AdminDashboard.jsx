...
export default function AdminDashboard({ onNavigate }) {
  ...
  const runSecurityMigration = async () => {
    ...
    try {
      const res = await api.post('/admin/migrate-security-columns');
  ...
  return (
    <div className="container max-w-6xl py-10 mx-auto px-4">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-8">
        <p className="text-sm text-stone-700 mb-2">One-time setup: adds login lockout tracking (stops PIN/password brute forcing).</p>
        <button
          onClick={runSecurityMigration}
