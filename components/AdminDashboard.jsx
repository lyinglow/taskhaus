...
export default function AdminDashboard({ onNavigate }) {
  ...
  const runMigration = async () => {
    ...
    try {
      const res = await api.post('/admin/migrate-long-description-column');
  ...
  return (
    <div className="container max-w-6xl py-8 mx-auto px-4">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-8">
        <p className="text-sm text-gray-700 mb-2">One-time setup: adds the long description column to the database.</p>
        <button
        ...
        {migrationStatus && <p className="text-sm text-gray-700 mt-2">{migrationStatus}</p>}
      </div>
