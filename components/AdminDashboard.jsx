...
export default function AdminDashboard({ onNavigate }) {
  ...
  const runDurationMigration = async () => {
    setMigrateStatus('Running...');
    try {
      const res = await api.post('/admin/migrate-estimated-duration');
  ...
  return (
    <div className="container max-w-6xl py-10 mx-auto px-4">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-8">
        <p className="text-sm text-stone-700 mb-2">One-time setup: adds the estimated duration field to services.</p>
        <button
          onClick={runDurationMigration}
